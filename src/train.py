...
Trains the Siamese U-Net on LEVIR-CD with:
    - Mixed precision
    - Cosine annealing LR schedule with linear warmup
    - Checkpointing best model by F1
    - Early stopping on validation F1 plateau
"""
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        # Mixed precision forward pass — float16 for speed, float32 for loss
        with autocast(device_type="cuda", enabled=device.type == "cuda"):
            logits = model(img_a, img_b)
            loss = criterion(logits, mask)

        # Scaled backward pass for mixed precision stability
...
def train(
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Checkpointing — save best by F1, not loss
